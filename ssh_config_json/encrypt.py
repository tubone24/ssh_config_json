...
class AESCipher(object):
    def __init__(self, key=None, key_num=32):
        if key is not None:
            ...
            self.key = (hashlib.md5(raw_key.encode('utf-8')).hexdigest()).encode('utf-8')

    @staticmethod
    def create_key(key_num=32):
        return ''.join([SystemRandom().choice(string.ascii_letters + string.digits) for i in range(key_num)])
    ...
    def decrypt(self, encrypted):
        ...
        data = Padding.unpad(cipher.decrypt(encrypted[AES.block_size:]), AES.block_size, 'pkcs7')
